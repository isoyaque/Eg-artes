...
// Função para exibir os produtos com base no filtro selecionado
function exibirProdutos(tipo) {
    // Limpar lista antes de adicionar produtos filtrados
    listaProdutos.innerHTML = '';

    // Filtrar produtos com base no tipo selecionado
    const produtosFiltrados = tipo === 'todos' ? produtos : produtos.filter(produto => produto.type === tipo);

    // Criar os elementos dos produtos filtrados
...
tipoProdutoSelect.addEventListener('change', (e) => {
    ...
    exibirProdutos(tipoSelecionado);
});
